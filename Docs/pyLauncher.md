...
First, make sure you have Python 3.11.14 installed, then, from the project root, runn:
``source pyLauncher/bin/activate``
You should then see your command line gain the prefix "(myenv) which is the internal name of the VE. If you run ``python --version``, you should see it return version 3.11. To leave the environment, simply type ``deactivate`` The best part is that since we already used this environment, all the dependencies should be already installed and ready to go!

## How do I make my own Virtual Environment?

If you're having problems with the existing PyLauncher or just want the learning experience, here's what you'll need to do: first install Python 3.11. Remember, it needs to be that specific version. Now, assuming it installed under the package name python3.11, run ``python3.11 -m venv [environment name]``, which will create a blank canvas for you. Then, you can install ``pip install -r OpenCV/requrements.txt``
